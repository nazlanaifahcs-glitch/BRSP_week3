...
* **Visualisasi & Output Hasil:**
    * **Volcano Plot (`plots/1_volcano_plot.png`):** Memetakan sebaran gen secara keseluruhan berdasarkan signifikansi statistik ($-\log_{10} \text{p-value}$) dan tingkat perubahan ekspresi ($\log_2 \text{Fold Change}$).
    * **Heatmap (`plots/2_heatmap_top50.png`):** Visualisasi pola klastering dan gradien ekspresi dari 50 gen teratas (paling signifikan) antara kelompok Dengue dan Kontrol.
    * **Gene Ontology / GO Plot (`plots/3_go_enrichment_plot.png`):** Analisis pengayaan fungsional gen pada domain *Biological Process* untuk melihat tugas biologis gen yang aktif.
    * **KEGG Pathway Plot (`plots/4_kegg_enrichment_plot.png`):** Memetakan gen-gen yang berubah ke dalam jalur metabolisme atau jalur sinyal penyakit yang spesifik di dalam sel manusia.
